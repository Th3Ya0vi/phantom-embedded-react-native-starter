...
//
//   // Find the user's Solana address
...
//           <ActionButton icon="⬆️" label="Send" />
//           <ActionButton icon="⬇️" label="Receive" />
...
//             value={`$${walletData.solValue.toFixed(4)}`}
//             color="#8b5cf6"
//           />
//         </View>
//
...
// );
//
// const TransactionRow = ({ type, desc, status, date }: any) => (
...
//   sectionTitle: { color: Colors.textPrimary, ...Typography.h2, marginBottom: 16 },
      ...
      export default function WalletScreen() {
        ...
        const handleSend = async () => {
          ...
          if (walletData.usdcBalance < parseFloat(amountToSend)) {
            Alert.alert("Insufficient Balance", `You need at least ${amountToSend} USDC.`);
            return;
          }

          Alert.alert(
            "Confirm Transfer",
            `Send ${amountToSend} ${tokenToSend} to GeSIM Vault?`,
            [
